...
import time
import logging
from functions.Mechanics import *


def Bragg_plotter(hmax, kmax, lmax, dthp, dthy, dthr, alpha, roll):
    p_angle_list=[]
    phen_list=[]
    label_list=[]
    linestyle_list=[]
...
def Bragg_plotter_roll(hmax, kmax, lmax, dthp, dthy, dthr, alpha, pitch):  
    r_angle_list=[]
    phen_list=[]
    label_list=[]
    linestyle_list=[]
    ...
    def plotene(thplist,fact,n,h,k,l,a,DTHP,thylist,thr,n0,pitchax,rollax,yawax):
        ...
        return thylist, eevlist, label, linestyle, gid, color
    ...
    #thylist=np.linspace(-4,4,300) 
    
    
    
    DTHP = dthp#-0.6921-0.09
    
    DTHY = dthy+(alpha*pitch)    #15#15#0#-0.15#-0.39#-0.15 #0.0885
    DTHR = dthr
    thylist=(-DTHY+np.linspace(-4,4,300))/180*np.pi                 
    thplist = np.ones(thylist.shape)*pitch - dthp
    #######AMERICAN YAW DEFINITION 
            ...
            for l in range(-lmax,lmax+1):
                ...
                if (h%2 and k%2 and l%2) or (not(h%2) and not(k%2) and not(l%2) and not((h+k+l)%4)) and not(h==0 and k==0 and l==0):
                    ...
                    if allowed==1: 
                        ...
                        counter=counter+1
                        phen_list.append(list(phen))
                        r_angle_list.append(list(180*thylist/np.pi+DTHY))
                        linestyle_list.append(str(linestyle))
                        gid_list.append(str(gid))
                        color_list.append(color)
    #plotene(thplist,fact,nord,3,3,7,a,DTHP,thy,thr,n0,pitchax,rollax,yawax)                
    return phen_list, r_angle_list, gid_list, linestyle_list, color_list         
